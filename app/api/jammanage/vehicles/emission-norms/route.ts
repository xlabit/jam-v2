...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching vehicle emission norms:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const emissionNorm = await prisma.vehicleEmissionNorm.create({
      data: {
        name: data.name,
    ...
    return NextResponse.json(emissionNorm, { status: 201 });
  } catch (error) {
    console.error('Error creating vehicle emission norm:', error);
