  ...
  try {
    ...
    if (existing.usageCount > 0 && data.name !== existing.name) {
      return NextResponse.json(
        {
          message:
            'Cannot change the name as this make is being used by vehicles. Please create a new make instead.',
        },
        { status: 400 },
      );
    }

    const existingWithName = await prisma.vehicleMake.findUnique({
...
export async function DELETE(
  ...
  try {
    ...
    if (make.usageCount > 0) {
      ...
      return NextResponse.json({
        message:
          'Make is being used and has been set to Inactive. It will no longer appear in selection lists.',
      });
    }
