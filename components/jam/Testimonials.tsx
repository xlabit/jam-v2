    ...
    <section className="jam-section bg-[var(--jam-neutral-50)]">
      <div className="jam-container">
        ...
        <div className="relative max-w-4xl mx-auto">
                    ...
                    <Star
                      ...
                      className={`w-5 h-5 ${
                        i < testimonials[currentIndex].rating
                          ? 'text-[var(--jam-accent)] fill-[var(--jam-accent)]'
                          : 'text-[var(--jam-neutral-300)]'
                      }`}
                    />
                  ))}
                </div>

                <blockquote className="jam-body text-gray-700 mb-6 italic">
                  "{testimonials[currentIndex].quote}"
                </blockquote>

                <div>
                  <div className="jam-h4 text-[var(--jam-neutral-900)]">
                    {testimonials[currentIndex].name}
                  </div>
                  <div className="jam-small text-gray-700">
          ...
          <button
            onClick={prev}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-16 w-12 h-12 bg-white rounded-full shadow-[var(--jam-elev-2)] flex items-center justify-center hover:bg-gray-100 transition-colors jam-focus-ring"
          ...
          <button
            onClick={next}
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-16 w-12 h-12 bg-white rounded-full shadow-[var(--jam-elev-2)] flex items-center justify-center hover:bg-gray-100 transition-colors jam-focus-ring"
            ...
            {testimonials.map((_, index) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all jam-focus-ring ${
                  index === currentIndex
                    ? 'w-8 bg-[var(--jam-primary)]'
                    : 'bg-[var(--jam-neutral-300)]'
                }`}
